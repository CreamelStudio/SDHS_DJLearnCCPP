...
{
	assert( m_hdc == NULL ); //dc�� (NULL)������ ���α׷� ����
	m_hdc = hdc;
	m_hdc = CreateCompatibleDC( hdc );
	HBITMAP	hBM = CreateCompatibleBitmap( hdc, rtSystem.right, rtSystem.bottom);
	SelectObject(m_hdc, hBM);
	m_OldPen = (HPEN)SelectObject( m_hdc, m_BlackPen );
	SelectObject( m_hdc, m_OldPen );
	m_OldBrush = (HBRUSH)SelectObject( m_hdc, GetStockObject( BLACK_BRUSH ) );
...
{
	BitBlt(hdc, 0, 0, rtSystem.right, rtSystem.bottom, m_hdc, 0, 0, SRCCOPY);
